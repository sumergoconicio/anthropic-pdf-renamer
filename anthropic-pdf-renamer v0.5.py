...
# extracts text from the first page, sends that text to Anthropic's Claude
...
def llm_guess(rip_text):
    """
    Sends text to Claude to get title, author, and publication date.
    """
    try:
        response = chai.messages.create(
            model="claude-3-haiku-20240307",  # Using Claude 3 Haiku - faster and more cost-effective
            max_tokens=1000,
            system="You are a librarian interested in the organization of knowledge. "
        ...
        # Check if the output is wrapped in ```json...``` or similar code blocks
        json_pattern = r"```(?:json)?\s*([\s\S]*?)\s*```"
        match = re.search(json_pattern, output_text)
        if match:
            output_text = match.group(1)
        
        # Clean any possible markdown or explanation text
        output_text = re.sub(r'^.*?(\{)', r'\1', output_text, flags=re.DOTALL)
        output_text = re.sub(r'(\}).*?$', r'\1', output_text, flags=re.DOTALL)
        
        return output_text.strip()
    except Exception as e:
        print(f"Error from Anthropic API: {e}")
        return None
...
def rename_pdf_based_on_title(pdf_path):
    ...
    if llm_guessed_response is None:
        print(f"Skipping {original_filename} due to Anthropic API failure.")
